...
export function CaptainFAB({ onClick }: CaptainFABProps) {
  return (
    <button
      onClick={onClick}
      aria-label="Consult Capt. Benjamin"
    ...
    >
      {/* CB monogram */}
      <span className="font-display text-base font-bold text-ink leading-none">CB</span>

      {/* Online indicator */}
      <span className="absolute right-0.5 top-0.5 flex h-4 w-4 items-center justify-center">
        <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-teal opacity-60" />
        <span className="relative h-2.5 w-2.5 rounded-full bg-teal border border-ink/30" />
      </span>
    </button>
  )
}
